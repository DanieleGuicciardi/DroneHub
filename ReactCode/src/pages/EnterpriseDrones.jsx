...
const EnterpriseDrones = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-white px-4 py-16">
      <h1 className="text-4xl font-bold mb-8 text-center">Enterprise Drones</h1>

      <div className="flex justify-center mb-10">
        <select
          value={selectedCategory}
          onChange={(e) => setSelectedCategory(e.target.value)}
          className="bg-gray-800 text-white px-4 py-2 rounded-md border border-gray-700 focus:outline-none focus:ring-2 focus:ring-cyan-500"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 max-w-6xl mx-auto">
        {filtered.map((product) => (
          <Link
            to={`/products/epdrones/${product.slug.current}`}
            key={product._id}
